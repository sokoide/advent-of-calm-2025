...
func ParseRichD2(content string) (*Architecture, error) {
	arch := &Architecture{
	...
	for scanner.Scan() {
		...
		if matches := flowStepPattern.FindStringSubmatch(line); matches != nil && currentFlow != nil {
			...
			currentFlow.Transitions = append(currentFlow.Transitions, Transition{
				SequenceNumber: seq,
		...
		if matches := relPattern.FindStringSubmatch(line); matches != nil {
			...
			currentRel = &Relationship{
				...
				RelationshipType: RelationshipType{
					Connects: &Connects{
						Source:      NodeInterface{Node: matches[1]},
						Destination: NodeInterface{Node: matches[2]},
					},
				},
			}
			continue
...
func parseNodeAnnotation(node *Node, key, value string) {
	switch key {
	case "id":
		node.UniqueID = value
	case "type":
...
func parseRelAnnotation(rel *Relationship, key, value string) {
	switch key {
	case "id":
		rel.UniqueID = value
	case "description":
	...
	case "encrypted":
		if value == "true" {
			rel.Encrypted = BoolPtr(true)
		} else {
			rel.Encrypted = BoolPtr(false)
		}
	case "classification":
