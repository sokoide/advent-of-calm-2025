...
func defineNodes(a *domain.Architecture) *nodesContainer {
	...
	nc.Admin = a.DefineNode(
		"admin",
		domain.Actor,
		"Admin",
		"A staff member who manages products and orders.",
	...
	nc.OrderSvc = a.DefineNode(
		...
		domain.WithControl(
			...
			domain.NewRequirement(
				...
				domain.NewCircuitBreakerConfig(50, 30, 10),
			),
		),
	)
	...
	nc.DBCluster = a.DefineNode(
		...
		domain.WithControl(
			...
			domain.NewRequirement(
				...
				domain.NewFailoverConfig(15, 5, true),
			),
		),
	)
	...
	nc.InventoryDB = a.DefineNode(
		"inventory-db",
		domain.Database,
		"Inventory Database",
		"Stores stock levels.",
		domain.WithOwner("dba-team", "CC-4000"),
		domain.WithMeta(
			domain.Merge(
				metaDBA,
				metaManagedSvc,
				map[string]any{"backup-schedule": "weekly at Sunday 03:00 UTC", "restore-time": "30 minutes"},
			),
		),
	)
...
func addOrderFlow(a *domain.Architecture, gw *domain.Node, l *linksContainer) {
	a.DefineFlow("order-processing-flow", "Customer Order Processing", "End-to-end flow from customer placing an order to payment confirmation").
		...
		Steps(
			...
			domain.StepSpec{
				ID:   l.GWToOrder[gw.UniqueID].GetID(),
				Desc: fmt.Sprintf("%s routes to Order Service", gw.Name),
			},
...
func addInventoryFlow(a *domain.Architecture, gw *domain.Node, l *linksContainer) {
	a.DefineFlow("inventory-check-flow", "Inventory Stock Check", "Admin checks and updates inventory stock levels").
		...
		Steps(
			...
			domain.StepSpec{ID: l.LBToGW[gw.UniqueID].GetID(), Desc: fmt.Sprintf("LB routes to %s", gw.Name)},
			domain.StepSpec{
				ID:   l.GWToInv[gw.UniqueID].GetID(),
				Desc: fmt.Sprintf("%s routes to inventory service", gw.Name),
			},
			...
			domain.StepSpec{
				ID:   l.GWToInv[gw.UniqueID].GetID(),
				Desc: "Return inventory report",
				Dir:  "destination-to-source",
			},
		)
}
