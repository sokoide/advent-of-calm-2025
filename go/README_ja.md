...
以下のコマンドで、フロントエンドのビルドと、**AI エージェント機能を含む**サーバーの起動が自動で行われます。
...
### CALM Studio UI (エディタのみ) の起動

AI エージェントを起動せず、軽量なエディタ機能のみを利用したい場合は、以下を使用します。

```bash
make studio-ui
```

`studio` はログに `agent:` / `studio:` のプレフィックスを付けて表示します。
...
-   **AI 支援**: 外部の AI アシスタントがポート `8787` を介してアーキテクチャ定義にアクセスし、設計の提案やコード生成を支援します。

### その他のターゲット

| コマンド             | 説明                                                         |
| :------------------- | :----------------------------------------------------------- |
| **`make format`**    | `gofmt` を使用して Go コードを整形します。  |
