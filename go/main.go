...
func addGeneralMetadata(a *Architecture) {
	...
	a.AddMeta("owner", "Architecture Team").
		...
		AddMeta("monitoring", map[string]any{
...
func addGlobalControls(a *Architecture) {
	a.Controls["security"] = &Control{
		...
		Requirements: []Requirement{
			...
			NewRequirementURL("https://internal-policy.example.com/security/tls-1-3-minimum", "https://configs.example.com/security/tls-config.yaml"),
		},
	}
	a.Controls["performance"] = &Control{
		...
		Requirements: []Requirement{
			...
			NewRequirementURL("https://internal-policy.example.com/performance/availability-target", "https://configs.example.com/infra/ha-config.yaml"),
		},
	}
	a.Controls["high-availability"] = &Control{
...
func addFlows(a *Architecture) {
	a.Flow("order-processing-flow", "Customer Order Processing", "End-to-end flow from customer placing an order to payment confirmation").
		AddMeta("business-impact", "Customers cannot complete purchases - direct revenue loss").
		AddMeta("degraded-behavior", "Orders queue in message broker; processed when service recovers").
		AddMeta("customer-communication", "Display 'Order processing delayed' message").
		AddMeta("sla", "99.9% availability, 30s p99 latency").
	...
	a.Flow("inventory-check-flow", "Inventory Stock Check", "Admin checks and updates inventory stock levels").
		AddMeta("business-impact", "Stock levels may be inaccurate - risk of overselling").
		AddMeta("degraded-behavior", "Fall back to cached inventory; flag orders for manual review").
		AddMeta("customer-communication", "Display 'Stock availability being confirmed'").
		AddMeta("sla", "99.5% availability, 500ms p99 latency").
...
func addNodes(a *Architecture) {
	...
	lb := a.Node("load-balancer", Service, "Load Balancer", "High-availability entry point that distributes traffic to API Gateways.")
	lb.Standard("CC-2000", "platform-team").
		AddMeta("tech-owner", "Network Team").AddMeta("owner", "platform-team").AddMeta("oncall-slack", "#oncall-platform").
		...
		AddMeta("dashboard", "https://grafana.example.com/d/lb-metrics").AddMeta("log-query", "service:load-balancer AND error").
		AddMeta("alerts", []string{"LB-HighLatency", "LB-TargetGroupUnhealthy"}).
		AddMeta("deployment-type", "managed-service").AddMeta("business-criticality", "high").AddMeta("ha-enabled", true)
	...
	for i := 1; i <= 2; i++ {
		...
		gw := a.Node(id, Service, name, desc).
			Standard("CC-2000", "platform-team").
			AddMeta("tech-owner", "Edge Team").AddMeta("owner", "platform-team").AddMeta("oncall-slack", "#oncall-platform").
		...
		gw.Controls["performance"] = gwPerf
	...
	orderSvc := a.Node("order-service", Service, "Order Service", "Handles order creation and lifecycle management.").
		...
		AddMeta("tier", "tier-1")
	orderSvc.Control("circuit-breaker", "Fault tolerance for downstream service calls",
	...
	orderSvc.Interface("order-db-write-client", "JDBC").SetName("Order DB Write Client").SetDesc("Writes to the primary database.")
	orderSvc.Interface("order-db-read-client", "JDBC").SetName("Order DB Read Client").SetDesc("Reads from the replica database.")
	...
	invSvc := a.Node("inventory-service", Service, "Inventory Service", "Manages product stock levels.").
		Standard("CC-4000", "inventory-team").
		...
		AddMeta("deployment-type", "container").
		AddMeta("failure-modes", invFailures).
	...
	paySvc := a.Node("payment-service", Service, "Payment Service", "Integrates with external payment providers.").
		Standard("CC-5000", "payments-team").
		...
		AddMeta("failure-modes", payFailures).
		AddMeta("health-endpoint", "/health").
		...
		AddMeta("tier", "tier-1")
	paySvc.Control("compliance", "PCI-DSS compliance for payment processing",
		NewRequirementURL("https://www.pcisecuritystandards.org/documents/PCI-DSS-v4.0", "https://configs.example.com/compliance/pci-dss-config.json"),
	)
	...
	dbMeta := map[string]any{"tech-owner": "DBA Team", "backup-schedule": "daily at 02:00 UTC", "restore-time": "60 minutes", "dba-contact": "dba-team@example.com", "deployment-type": "managed-service", "data-classification": "PII", "replication-mode": "async"}

	a.Node("order-database-primary", Database, "Order Database (Primary)", "Main writable database for orders.").
		Standard("CC-3000", "dba-team").
		AddMeta("role", "primary").
		AddMeta("backup-schedule", dbMeta["backup-schedule"]).
		AddMeta("data-classification", dbMeta["data-classification"]).
		AddMeta("dba-contact", dbMeta["dba-contact"]).
		AddMeta("deployment-type", dbMeta["deployment-type"]).
		AddMeta("replication-mode", dbMeta["replication-mode"]).
		AddMeta("restore-time", dbMeta["restore-time"]).
		AddMeta("tech-owner", dbMeta["tech-owner"]).
	...
	a.Node("order-database-replica", Database, "Order Database (Replica)", "Read-only replica for scaling read operations.").
		...
		AddMeta("restore-time", dbMeta["restore-time"]).
		AddMeta("tech-owner", dbMeta["tech-owner"]).
		Interface("order-sql-replica", "JDBC").SetPort(5432).SetDB("orders_v1").SetHost("orders-replica.example.com")
...
func addRelationships(a *Architecture) {
	...
	a.Connect("order-connects-primary-db", "Order Service persists data to Primary Order Database.", "order-service", "order-database-primary").
		SrcIntf("order-db-write-client").DstIntf("order-sql-primary").Data("confidential", true).AddMeta("monitoring", true)
	a.Connect("order-publishes-to-queue", "Order Service publishes payment task to queue.", "order-service", "order-queue").
		SrcIntf("payment-publisher").WithProtocol("AMQP").Data("internal", true)
	a.Connect("payment-subscribes-to-queue", "Payment Service consumes payment task from queue.", "order-queue", "payment-service").
		DstIntf("payment-consumer").WithProtocol("AMQP").Data("internal", true)
	a.Connect("order-connects-replica-db", "Order Service reads data from Replica Order Database.", "order-service", "order-database-replica").
		SrcIntf("order-db-read-client").DstIntf("order-sql-replica").Data("confidential", true).AddMeta("monitoring", true)
	a.Connect("order-connects-inventory", "Order Service checks/reserves stock in Inventory Service.", "order-service", "inventory-service").
		SrcIntf("order-inventory-client").DstIntf("inventory-api").Data("internal", true).AddMeta("monitoring", true).AddMeta("circuit-breaker", true).AddMeta("latency-sla", "< 100ms")
	a.Connect("inventory-connects-db", "Inventory Service manages stock in Inventory Database.", "inventory-service", "inventory-db").
		SrcIntf("inventory-db-client").DstIntf("inventory-sql").Data("internal", true).AddMeta("monitoring", true)
