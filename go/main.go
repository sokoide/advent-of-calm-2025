...
func defineNodes(a *Architecture) *NodesContainer {
	...
	nc.System = a.DefineNode("ecommerce-system", System, "E-Commerce Platform", "The overall e-commerce system containing microservices.",
	...
	nc.Broker = a.DefineNode("message-broker", System, "Message Broker (RabbitMQ)", "Central messaging system for failure isolation and async processing.",
		WithOwner("platform-team", "CC-2000"),
	...
	nc.DBCluster = a.DefineNode("order-database-cluster", System, "Order Database Cluster", "High-availability database cluster for order data.",
	...
	nc.OrderPrimary = a.DefineNode("order-database-primary", Database, "Order Database (Primary)", "Main writable database for orders.",
		WithOwner("dba-team", "CC-3000"), WithMeta(Merge(metaDBA, metaManagedSvc, map[string]any{"backup-schedule": "daily at 02:00 UTC", "restore-time": "60 minutes", "data-classification": "PII", "replication-mode": "async", "role": "primary"})))
	...
	nc.OrderReplica = a.DefineNode("order-database-replica", Database, "Order Database (Replica)", "Read-only replica for scaling read operations.",
		WithOwner("dba-team", "CC-3000"), WithMeta(Merge(metaDBA, metaManagedSvc, map[string]any{"backup-schedule": "daily at 02:00 UTC", "restore-time": "60 minutes", "data-classification": "PII", "replication-mode": "async", "role": "replica"})))
	...
	nc.InventoryDB = a.DefineNode("inventory-db", Database, "Inventory Database", "Stores stock levels.",
		WithOwner("dba-team", "CC-4000"), WithMeta(Merge(metaDBA, metaManagedSvc, map[string]any{"backup-schedule": "weekly at Sunday 03:00 UTC", "restore-time": "30 minutes"})))
...
func defineFlows(a *Architecture, n *NodesContainer, l *LinksContainer) {
	...
	flowTasks := []func(gw *Node){
		func(gw *Node) { addOrderFlow(a, gw, l) },
		func(gw *Node) { addInventoryFlow(a, gw, l) },
	}
...
func addOrderFlow(a *Architecture, gw *Node, l *LinksContainer) {
	a.DefineFlow("order-processing-flow", "Customer Order Processing", "End-to-end flow from customer placing an order to payment confirmation").
		...
		Steps(
			...
			StepSpec{ID: l.LBToGW[gw.UniqueID].GetID(), Desc: fmt.Sprintf("LB routes to %s", gw.Name)},
			StepSpec{ID: l.GWToOrder[gw.UniqueID].GetID(), Desc: fmt.Sprintf("%s routes to Order Service", gw.Name)},
...
func addInventoryFlow(a *Architecture, gw *Node, l *LinksContainer) {
	a.DefineFlow("inventory-check-flow", "Inventory Stock Check", "Admin checks and updates inventory stock levels").
		...
		Steps(
			...
			StepSpec{ID: l.LBToGW[gw.UniqueID].GetID(), Desc: fmt.Sprintf("LB routes to %s", gw.Name)},
			StepSpec{ID: l.GWToInv[gw.UniqueID].GetID(), Desc: fmt.Sprintf("%s routes to inventory service", gw.Name)},
			...
			StepSpec{ID: l.GWToInv[gw.UniqueID].GetID(), Desc: "Return inventory report", Dir: "destination-to-source"},
		)
}
